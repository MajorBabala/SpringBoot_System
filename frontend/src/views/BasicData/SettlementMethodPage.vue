  ...
  <div class="business-page">
    <div class="module-hero">
      <div>
        ...
        <div class="module-title">结算方式管理</div>
        <div class="module-desc">维护现金、转账、第三方支付等结算方式，为报销和后续单据扩展提供基础支持。</div>
      </div>
    ...
    <el-card class="app-card module-card">
      <template #header>
        <div class="card-header">
          <span>结算方式列表</span>
          <el-button type="primary" @click="openCreate">新增结算方式</el-button>
        </div>
      </template>
      ...
      <div class="table-shell">
        <el-table :data="tableData" border>
          <el-table-column prop="methodCode" label="方式编码" width="140" />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑结算方式' : '新增结算方式'" width="700px">
      <el-form :model="form" label-width="110px">
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="方式编码">
              <el-input v-model="form.methodCode" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
import { createSettlementMethod, deleteSettlementMethod, listSettlementMethods, updateSettlementMethod } from '../../api/settlementMethods'
...
const fetchList = async () => {
  loading.value = true
  try {
    const resp = await listSettlementMethods(query)
    if (resp.code === 200) {
      tableData.value = resp.data.records || []
      total.value = resp.data.total || 0
      return
    }
    ElMessage.error(resp.message || '结算方式加载失败')
  } catch (error) {
    ElMessage.error(error?.response?.data?.message || '结算方式加载失败')
  } finally {
    loading.value = false
  }
}
...
const submit = async () => {
  if (!form.methodCode || !form.methodName) {
    ElMessage.error('请填写方式编码和方式名称')
    return
  }
  saving.value = true
  try {
    const payload = { ...form }
    const resp = form.id
      ? await updateSettlementMethod(form.id, payload)
      : await createSettlementMethod(payload)
    if (resp.code === 200) {
...
const remove = async (id) => {
  try {
    await ElMessageBox.confirm('确认删除该结算方式吗？', '提示', { type: 'warning' })
    const resp = await deleteSettlementMethod(id)
    if (resp.code === 200) {
